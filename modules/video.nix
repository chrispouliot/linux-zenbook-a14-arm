...
let
  ...
  # is installed too so that experimental.video.firmwareName can select it
  # for a test without changing the firmware directory.
  ...
  '';
in {
  config = lib.mkIf cfg.experimental.video.enable {
