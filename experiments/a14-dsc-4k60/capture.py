...
def aux_read(device):
    ...
    ranges = [('receiver', 0x000, 16), ('dsc_caps', 0x060, 16),
              ('fec_caps', 0x090, 1), ('link_config', 0x100, 3),
              ('fec_config', 0x120, 1), ('dsc_enable', 0x160, 1), ('fec_status', 0x280, 1),
              ('extended_receiver', 0x2200, 16), ('repeater_caps', 0xf0000, 8)]
    result = {}
...
def main():
    ...
    print('A14 DSC 4K60 test recorder v1 — read-only', flush=True)
    ...
    with tempfile.TemporaryDirectory(prefix='a14-dsc-baseline-') as tmp:
        ...
        for path in [debug / 'state'] + sorted(debug.glob('*/dp_debug')):
