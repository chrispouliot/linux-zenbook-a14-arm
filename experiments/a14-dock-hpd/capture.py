...
def aux_read(device):
    ...
    ranges = [('repeater_caps', 0xf0000, 8), ('receiver', 0x000, 16), ('dsc_caps', 0x060, 16),
              ('fec_caps', 0x090, 1), ('link_config', 0x100, 3),
              ('fec_config', 0x120, 1), ('dsc_enable', 0x160, 1), ('fec_status', 0x280, 1),
              ('link_status', 0x202, 6), ('sink_power', 0x600, 1),
              ('extended_receiver', 0x2200, 16)]
    result = {}
...
def main():
    ...
    print('A14 dock IRQ containment recorder v1 — read-only', flush=True)
    ...
    with tempfile.TemporaryDirectory(prefix='a14-dsc-baseline-') as tmp:
        ...
        for name in ('a14_dp_dsc_test', 'a14_dp_dsc_144_test', 'a14_dp_dsc_wake_test', 'a14_dp_aux_wake_test'):
        ...
        run('system/gnome-shell-journal.log', ['journalctl', '-b', '--since=-15min',
            '-o', 'short-monotonic', '--no-pager', '_COMM=gnome-shell'])
        ...
        for entry in sorted(Path('/sys/class/drm_dp_aux_dev').glob('drm_dp_aux*')):
            ...
            try:
                ...
                continue
            print(f'Reading {connector} DSC/FEC capability registers via {entry.name}...', flush=True)
            run('aux/' + connector + '-' + entry.name + '.jsonl',
                [sys.executable, str(Path(__file__).resolve()), '--aux-child', str(device)], timeout=15)
...
if __name__ == '__main__':
    if len(sys.argv) == 3 and sys.argv[1] == '--aux-child':
        aux_read(sys.argv[2])
    else:
        main()
