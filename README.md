...
| Hardware video decode | Experimental, opt-in, untested | Ported upstream Glymur Iris video codec series behind `experimental.video.enable`; needs the optional `qcvss8480.mbn` firmware. See [video notes](docs/hardware.md#video-decode-experimental). |
...
| [Iris video codec backports](patches/video/) — opt-in | Thirteen commits from the upstream "media: iris: Add support for glymur platform" v10 series, ported onto the pinned snapshot: context-bank devices, PAS firmware loading through a Linux-managed IOMMU, per-block clock and power tables, the Glymur power sequence and dual-core selection, Glymur platform data, and the Glymur/CRD device-tree nodes. Enabled by `experimental.video.enable`. |
| [`a14-iris.dtsi`](patches/a14-iris.dtsi) — opt-in, untested | Enables the video codec node with PAS-authenticated firmware, mirroring the CRD. |
| [Video firmware handling](modules/video.nix) — opt-in | Installs the generic linux-firmware VPU image; the OEM `qcvss8480.mbn` is an optional manifest entry installed when present. |
