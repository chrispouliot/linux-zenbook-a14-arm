...
def main():
    ...
    print('A14 display recorder v1 — read-only', flush=True)
    ...
    if args.delay:
        ...
        try:
            ...
            return
    errors = []
    with tempfile.TemporaryDirectory(prefix='a14-display-capture-') as tmp:
        folder = Path(tmp)
        ...
        for directory in ('patches', 'kernel', 'display', 'tools', 'docs/display', 'experiments'):
            sources += sorted((repo / directory).rglob('*'))
        total = 0
        for src in sources:
            if not src.is_file():
                continue
        ...
        if shutil.which('git'):
            ...
            run('local-repo/status.txt', git + ['status', '--short', '--untracked-files=all',
                                               '--', 'kernel.nix', 'flake.nix', 'flake.lock', 'patches', 'kernel', 'display', 'tools', 'docs/display', 'experiments'])
            run('local-repo/diff-from-head.patch', git + ['diff', '--no-ext-diff', '--no-textconv',
                                                        'HEAD', '--', 'kernel.nix', 'flake.nix', 'flake.lock', 'patches', 'kernel', 'display', 'tools', 'docs/display', 'experiments'])
        ...
        for name in ('a14_dp_dsc_test', 'a14_dp_dsc_144_test', 'a14_dp_dsc_wake_test', 'a14_dp_aux_wake_test', 'a14_edp_retry_test', 'a14_dp_pair_config_test', 'a14_dp_sink_power_test', 'a14_dp_state_trace_test', 'a14_dp_config_reuse_test', 'a14_dp_repeater_restore_test', 'a14_dp_sink_cleanup_test', 'a14_dp_first_aux_probe_test', 'a14_dp_bounded_cleanup_test', 'a14_edp_bpp_init_test', 'a14_dp_repeater_reset_test', 'a14_dp_aux_diag_test', 'a14_dp_transparent_test', 'a14_edp_depth_test'):
        ...
        run('system/boot-recovery-unit.txt', ['systemctl', 'cat', 'a14-dock-boot-recovery.service', '--no-pager'])
        recovery_state = Path('/run/a14-dock-boot-recovery')
        if recovery_state.is_dir():
            for entry in sorted(recovery_state.iterdir()):
                if entry.is_file() and not entry.is_symlink():
                    if entry.stat().st_size <= 16 * 1024 * 1024:
                        copy(entry, 'boot-recovery-state/' + entry.name)
                    else:
                        errors.append('Skipped oversized recovery state: ' + entry.name)
